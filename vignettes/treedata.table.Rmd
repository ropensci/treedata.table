...
`treedata.table` includes additional functions that allow the identification of `discrete` and `continuous` character in a given dataset. For instance, the `detectCharacterType()` function can be used to examine whether `SVL` is `discrete` or `continuous`: 
